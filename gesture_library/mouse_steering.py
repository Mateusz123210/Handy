...
from gesture_library.mouse_move_frame import MouseMoveFrame


class GestureMouseSteering:

    def __init__(self, camera, win_reference):
        self.mouse_frame = MouseMoveFrame()
        self.active = True
        self.cam = camera
    ...
    def start_mouse_steering(self, reference):
        ...
        while True:
            ...
            self.mouse_frame.calculate_mouse_move_frame(flip_img)
            ...
            if results.multi_hand_landmarks:
                for hand_lms in results.multi_hand_landmarks:
                    ...
                    if points[8][1] < points[5][1] and points[17][0] > points[1][0]:
                        if self.mouse_frame.check_if_in_frame(points[8][0], points[8][1]) is True:
                            mouse_x, mouse_y =  self.calculate_next_mouse_position(points)
    ...
    def calculate_next_mouse_position(self, points):
        mouse_x = int(
            float(points[8][0] - self.mouse_frame.get_left_bound()) / self.mouse_frame.get_frame_width_float() * \
                float(self.monitors[self.current_monitor_number][2])) + self.monitors[self.current_monitor_number][0]
        mouse_y = int(
            float(points[8][1] - self.mouse_frame.get_upper_bound()) / self.mouse_frame.get_frame_height_float() * \
                float(self.monitors[self.current_monitor_number][3])) + self.monitors[self.current_monitor_number][1]
